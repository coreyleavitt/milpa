...
- [ ] S-RustCrypto — additive two-file vendor patch + wire stages 5-7 (D7). **EXPLORE FIRST next turn:** (1) confirm `.vendor-sigstore` present + the cargo `[patch.crates-io]` stanza; (2) the D7 shape = extract the ~90-line `verify_digest` body parameterized over digest source (`&[u8]` vs `Sha256`), expose on BOTH async `Verifier` + `blocking::Verifier` (additive, two-file — NOT #183's subtractive shape); wire `entry_trust.rs:329` stages 5-7 (reuse `rekor_adapter::verify_entry_inclusion`), delete the unconditional `SignatureInvalid`; (3) **TEST-FIXTURE question:** its test needs a REAL Layer-2 bundle to verify Trusted — check how Python's `SigstoreEntryVerifier` is tested (what real per-entry bundle fixture it uses, e.g. an `_oracle/attestation/` entry) and whether Rust can reuse it, since S6 (real-crypto minting) is LATER + Corey-gated. If no real Layer-2 fixture exists pre-S6, S-RustCrypto may only get the wiring + negative-vector tests, deferring the positive real-bundle verify to S6 — decide during exploration.
