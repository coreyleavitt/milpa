...
- [~] S-EpochCommitment — **Python half DONE** `17e7df4` (full suite exit 0); **Rust half REMAINING**. New `impls/python/milpa/epoch_commitment.py`; wired in `cli.py:_apply_epoch_commitment_phase` inside `_load_index_for_verb` (after index-trust, before selection). Both slugs in errors.py+errors.md (bijection green).
