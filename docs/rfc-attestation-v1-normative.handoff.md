...
3. [x] **S4 — DONE `8b881d5`** (Python exit 0; Rust workspace 0 failed). Flipped index-trust + entry-trust defaults→strict (index-history stays warn) in both impls + SSOT table + cli-contract + errors.md. **Feas-C1 carve-out FIXED** (`_fixture_entry_trust_config`/runner.rs now build the gate at effective policy for every manifest → entry-trust=strict exercised corpus-wide; safe via Unarmed⇒warn-equiv). Index-trust flip proven via prove-the-default unit tests (RFC "or"). **Migration = 0 fixtures** (NOT ~228): the general conformance adapter `_build_env` never invokes the index-trust gate (parse_index directly; index-trust tested only via dedicated 338+ fixtures w/ explicit policy) — RFC's 228 assumption was wrong about the harness. **SSOT fix:** Workspace derived Default (gave warn) → hand Default impl matching parse (strict); Manifest doesn't derive Default. Fixed ~16 test stragglers hardcoding the old warn default (cli_index_trust ×3, workspace default test, manifest format round-trips ×12).
   **⚠️ FOLLOW-UP for Corey (finding, non-blocking):** the general conformance corpus does NOT exercise index-trust=strict end-to-end (adapter bypasses it). If you want corpus-level index-trust proof, that's the ~228-fixture migration (option b) — deferred; the flip is proven via dedicated fixtures + unit tests (option a). Also: the `_execute_index_trust_fixture` harness still defaults `MILPA_INDEX_TRUST_MANIFEST` to `"warn"` (inert — all index-trust fixtures set it explicitly).
...
- [x] S9 — DONE. Doc reconcile: comparison-vs-nimble-atlas.md (attestation SHIPPED strict-default v1 + self-hosted `index-trust "off"` D11 note), rfc-registry-trust-federation.md §6.1 superseded-pointer (default→strict), RFC status header → IMPLEMENTED. errors.md bijection confirmed green (both new slugs, both impls). **Issues filed: #195** (extend black-box runner + full ~207-fixture corpus migration + gate on harness/), **#196** (break-glass lever), **#197** (upstream sigstore-rs raw-digest patch). Memory updated ([[v1_critical_path]] = COMPLETE) + MEMORY.md compacted. **Cross-repo remaining (Corey/tianguis): live production epoch re-arm.** ALL 14 SLICES DONE.
