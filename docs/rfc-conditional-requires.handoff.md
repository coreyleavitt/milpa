...
- **Stage:** 3 (/tdd grind) — **S1+S2+S3a+S3b+S4 done; S5 next**   •   **Round:** —
...
- [ ] **S5 NEXT** — spec docs (no code beyond warning text). Per RFC §9 S5: `dep-decl.md §7.5` (the §3.1 grammar table + negation/poison algebra + warning-on-UNRECOGNIZED; **fold in the S1 generalization: single-`NimMajor` accepts all 5 operators**), `dep-decl.md §1` (predicates field on RequireEntry), `manifest-grammar.md §5.3` (mirror — recognized `when` translated, warning only on unrecognized), `lockfile-schema.md` (the additive `cond-require` node: inline + `{ when }` block, `(not)` negation, ordering; + the §7.5-activation→#110 addendum). Update in-code warning text if needed. **NO spec/lockfile-version bump** (additive, pre-stabilization). Spec is prose — likely no test gate, but run both suites after to be safe.
- [ ] **S6** — conformance corpus (the four-runner + harness validation slice). Author §6 fixtures (cond-require scenarios incl. elif/else, URL/dev-deps, unrecognized-warns, mixed graph); fixture-138 already updated. Add `lockfile.cond-require` coverage clause **atomically** with fixture dirs (else `test_inventory_fully_covered` reddens). **Rebuild rust release** (`./dev-rust build --release -p milpa-cli`) before `python3 -m harness`; verify ALL FOUR runners + zero divergence + coverage 100%.
