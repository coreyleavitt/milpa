...
- [~] A3 — Phase A committed+pushed to milpa main (`2c89fad`). tianguis git path DONE: `vendor/driver.nim:84` `computeContentHash(tmp)` → `milpa hash git=<url> ref=<sha>` (execCmdEx, strip → `dag-sha256:`). Audit: `namespace.nim`/`vendor/merge.nim` CLEAN (no hasher use; only `IdentityDrift` namespace types). **BLOCKED — WRONG-SPEC**: RFC A3 said route BOTH drivers via `git=`, but `vendor/realdriver.nim:31` hashes an **OCI**-pulled+tar-extracted tree (`computeContentHash(extracted)`); `milpa hash` has no `oci=` form. Can't atomically delete identity.nim while realdriver+test_identity import it. FIX (goal-determined, in-RFC-intent): add `oci=<registry>/<repo>@<digest>` → `OciProvenance` to milpa `source_spec.py`+`source_spec.rs` (OciFetcher already returns identity), correct RFC A3 wording, re-push milpa, then finish A3 (reroute realdriver, delete identity.nim+test_identity.nim+identity_conformance.json). Awaiting Corey on sequencing / whether OCI publish path is live.
