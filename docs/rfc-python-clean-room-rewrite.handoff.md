...
**Done:** Stages 0–10 — the entire clean-room rewrite. `MILPA_PYTHON_NG=1 python -m harness` → python-ng 121/121, FAIL=0, known-failing=0, **ZERO divergence vs Rust**. pytest 1171/5/0, mypy --strict + ruff clean. ALL uncommitted (Corey-gated).
...
2. **`--certificate` flag** (RFC §8 external MUST): needs two SHARED-SPEC sections authored FIRST — `cli-contract.md` (`--certificate <path>` global flag, exit-0 no-slug) + `conformance-fixtures.md` (`check-certificate` fixture type). Then impl the flag in cli.py (serializer `certificate_to_json` already exists in solver.py, SSOT) + add fixtures. Cross-impl (Rust should emit the same JSON — reconcile to spec §5). **Wants Corey's eyes on the spec design.**
