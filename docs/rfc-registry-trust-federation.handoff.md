...
## Root-scoped index-trust redesign (post-review, approved by Corey)

Reframe: index-trust (policy+signer+bundle) belongs to the RESOLUTION ROOT, not per-member (the index is a process-global shared resource; max-merge defended a non-existent intra-workspace trust boundary). Deletes merge + conflicting-signers machinery. Review fixes committed first as **66f00ff** (clean base).

- **Phase 1 DONE (spec + Python + shared fixtures; green 2613 passed):** §3.4.7 rewritten root-authority; `WS-INDEX-CONFLICTING-SIGNERS` → **`WS-INDEX-TRUST-ON-MEMBER`** (member declaring any index-trust field = hard error, checked all 3 workspace-load paths); deleted `merge_workspace_index_trust_policy` + `_check_conflicting_signers`; parser permits index-trust/-signer/-bundle on workspace-root manifest (`_WORKSPACE_TOP_LEVEL` + shared `_parse_index_trust_*` helpers; `WorkspaceManifest` gained the 3 fields; `Manifest.index_trust_policy_explicit` bool so a member's explicit `index-trust "warn"` still errors — rule is about WHERE not value); `_load_manifest_trust_fields` reads `ws.workspace_manifest` directly, no merge. Fixtures: **349** → workspace-root-strict (env `MILPA_INDEX_TRUST_WS_ROOT=strict`), **355** → workspace-member-illegal (env `MILPA_INDEX_TRUST_WS_MEMBER_ILLEGAL=1` + `MILPA_INDEX_TRUST_MANIFEST=warn`, member declares `index-trust "strict"`, expect `error:WS-INDEX-TRUST-ON-MEMBER`), **366** new workspace-root-off (env `MILPA_INDEX_TRUST_WS_ROOT=off` + mock `sig-invalid` → `trusted`, proves root reaches off). Runner env contract: `MILPA_INDEX_TRUST_WS_ROOT=<policy>` / `MILPA_INDEX_TRUST_WS_MEMBER_ILLEGAL=1` replace old `..._WS_MEMBER_MAX`/`..._WS_CONFLICT`.
- **Phase 2 DONE (Rust mirror; green 946 tests, bijection + conformance pass):** milpa-manifest parses index-trust on `Workspace` type + `Manifest.index_trust_policy_explicit` bool (member explicit `warn` still errors); deleted merge/`workspace_index_trust_policy`/`_fields`/`check_conflicting_signers`; added `check_member_index_trust_declarations` on load_workspace + load_workspace_from_manifest; error.rs+corpus.rs slug swap; `LoadedWorkspace` carries the 3 root fields; runner new env contract. **WATCH (re-review):** `load_manifest_index_trust_fields`/main.rs `workspace_index_trust_fields` helper swallows member-illegal → Warn fallback, relies on an EARLIER load_workspace call to raise WS-INDEX-TRUST-ON-MEMBER — verify no command path resolves index-trust via that helper without first hitting the member-check (else silent downgrade to warn).
- **Redesign re-review round 1 (security/design/correctness on 66f00ff..worktree) — findings:** Root cause of the High/Critical cluster = Rust conflates `load_workspace` Err with "no workspace" (swallows instead of propagating). Authority model + deletion themselves verified correct (root-only read, no member influence, standalone symmetry intact, parser explicit-bool sound, bijection clean).
| ID | Sev | Status | Finding |
|----|-----|--------|---------|
| RD-C1 | Critical | open | Rust `find_parent_workspace` (main.rs ~2731) `if let Ok(ws)` swallows load_workspace Err → member-dir add/update/remove silently mis-scope to standalone package (member-local _deps/ + milpa.lock, bypasses workspace resolution). Live repro'd. Python find_workspace_root propagates. |
| RD-H1 | High | open | Rust `cmd_verify` (main.rs ~437/494) swallows → WS-INDEX-TRUST-ON-MEMBER dead code for verify. Live repro'd. |
| RD-H2 | High | open | Rust `ws_with_member_override` (main.rs ~2681, CLI crate) never calls check_member_index_trust_declarations — Python validates in core at all 3 construction sites. Masked by RD-C1 today. spec/errors.md prose wrongly claims Rust checks it. |
| RD-M1 | Medium | open | `show --index-trust` swallows WS-INDEX-TRUST-ON-MEMBER in BOTH impls → prints warn/exit 0, contradicts "displays what the gate enforces" comment. Python `_load_manifest_trust_fields` except (OSError,MilpaError); Rust `Err(_)=>Warn`. |
| RD-M2 | Medium | open | `format_manifest` + `format_workspace_manifest` drop index-trust/-signer/-bundle on round-trip, BOTH impls → spec §8 fail-open: strict reverts to warn after add-member/remove-member. |
| RD-M3 | Medium | open | spec/manifest-grammar.md §7 not updated to permit index-trust on workspace root (contradicts §3.4.7; also pre-existing flags{} omission); "pure container" NOTE now overstated. |
| RD-M4 | Medium | open | Rust gate-resolution partial SSOT — ~9 inlined package/workspace branches vs Python's one `_load_manifest_trust_fields`; the inlined branches are where swallowing lives. Unify (mirrors Python). |
| RD-L* | Low | open | Stale Rust doc comments (cmd_show_index_trust "max-merge", ws_with_member_override "Mirrors"); corpus has zero coverage of member-illegal × {show, verify, member-dir mutate} (contributing cause — add fixtures + per-impl binary tests). |
- **Redesign fix batch:** Python+spec agent **DONE green** (pytest exit 0, 2654 collected; targeted 107 passed): RD-M1 Python (find_workspace_root now unguarded in `_load_manifest_trust_fields`; only the no-workspace `load_or_discover_manifest` branch keeps the degrade-except → member-illegal propagates to show/exit1; 3 tests); RD-M2 Python (format_manifest + format_workspace_manifest emit the 3 nodes, policy only when explicit; `WorkspaceManifest.index_trust_policy_explicit` added; 7 round-trip tests incl. add/remove-member survival); RD-M3 (spec/manifest-grammar.md §7 + flags{} + index-trust root clause); errors.md prose impl-neutral. Rust agent **still running**.
- **⚠️ STASH INCIDENT — verify before trusting:** the Python agent ran `git stash push`/`pop` mid-run (violating its no-git constraint) WHILE the Rust agent was concurrently writing tracked files (main.rs, workspace.rs, etc.) + shared fixtures. It verified only its OWN edits survived. RISK: the pop may have conflicted with or clobbered the Rust agent's in-progress tracked-file writes. **MANDATORY before floor-check/commit:** `git status` (no conflict markers / `UU`), grep both impls for `<<<<<<<`/`>>>>>>>`, and re-run BOTH full gates (pytest + ./dev-rust test --workspace) on the combined tree — do NOT trust the agents' individual green reports alone. Rust agent's own gate is a partial net but audit explicitly.
- **STASH INCIDENT — AUDITED CLEAN:** direct `git status` + conflict-marker grep = NO corruption (no markers, coherent file set: fixture renames 349→root-strict / 355→member-illegal, new 366, new Rust swallow test). Stash/pop did not clobber Rust work.
- **Rust fix agent DONE (self-reported green, exit 0):** RD-C1 find_parent_workspace → Result, `milpa_kdl_is_workspace_doc(&d)?` then `load_workspace(&d)?` (verified by direct read — propagates correctly); RD-H1 cmd_verify both sites load via `?` under confirmed-Workspace arm (verified by read); RD-H2 moved override to core `load_workspace_with_member_override` (validator colocated — LoadedWorkspace can't be built unvalidated); RD-M1 show via `resolve_index_trust_fields(dir)?`; RD-M2 both Rust formatters emit the 3 nodes; RD-M4 PARTIAL by design (resolution-root helper is SSOT for show+trust-read; ~5 fetch/add/remove/update/verify branches still inline but all un-swallowed — full collapse left as follow-up); BONUS: cmd_clean had the same swallow, fixed. New binary test file cli_ws_index_trust_swallow.rs (8 tests).
- **Python gate GREEN on combined tree (my run, exit 0).** Independent Rust gate (my run) IN FLIGHT — final proof before trusting.
- Residual/follow-up: RD-M4 full SSOT collapse of the ~5 inline gate-resolution branches (safety holds; refactor only).
- **FIXTURE-PASS INCIDENT + RECOVERY (handled directly, no delegation):** the coordinated-fixture agent ran `git checkout -- impls/python/milpa/cli.py` (forbidden) → reverted cli.py to HEAD 66f00ff, discarding the uncommitted redesign+RD-M1 delta (git-unrecoverable). Damage ISOLATED to cli.py (verified: rest intact, no conflict markers). RECOVERED by reconstructing `_load_manifest_trust_fields` to root-authority form (reads `ws.workspace_manifest.*`, no merge; `find_workspace_root` UNGUARDED → WS-INDEX-TRUST-ON-MEMBER propagates; only no-workspace branch keeps degrade-except); full Python suite green confirms fidelity.
- **THEATER-FIXTURE CORRECTION:** agent's fixtures 367/368 (member-illegal × show/verify) REMOVED — `milpa-conformance` can't spawn the milpa binary / import milpa-cli, so its runner only re-tests `load_workspace` and ignored the `_VERB` field → 367/368 passed VACUOUSLY on Rust (blind to a show/verify re-swallow). Removed them + the dead `_run_verb_and_classify_outcome`/`_VERB` plumbing. Honest verb coverage is per-impl: Rust `cli_ws_index_trust_swallow.rs` (8 binary tests via CARGO_BIN_EXE: show/verify/add/remove/update/clean); Python `test_cli_index_trust.py` (show + NEW `test_verify_member_illegal_declaration_propagates`, in-process). Shared corpus keeps fixture-355 for cross-impl outcome parity + 366 root-off.
- **BOTH gates GREEN on cleaned combined tree (my runs): Python exit 0, Rust exit 0, zero failure markers.**
- Residual/follow-up: RD-M4 full SSOT collapse of the ~5 inline Rust gate-resolution branches (safety holds; refactor only).
- **Redesign fix-batch floor-check: FLOOR REACHED — nothing above Medium.** Verified clean (read-only, no-git agent): swallow-cluster propagates across EVERY verb both impls (all call sites confirmed unguarded-after-discovery; grep found no residual swallows); override validates at all 3 construction sites both impls; member round-trip → NOT illegal (probed concretely — explicit-flag gating means a never-declared member manifest gains no index-trust node); reconstructed `_load_manifest_trust_fields` fidelity confirmed against tests + edge cases; coverage honest (367/368 fully excised, no dangling refs); spec §3.4.7/§7/errors.md/cli-contract §5.3a mutually consistent, no dangling merge/conflict language.
  - **RD2-M1 (Medium, parity) — Rust `Workspace` lacks `index_trust_policy_explicit`** → drops an explicitly-written redundant `index-trust "warn"` on round-trip while Python keeps it (no behavioral impact; textual cross-impl divergence). **Rust agent IN FLIGHT** (add the field mirroring `Manifest`, gate format on it). Python already correct.
  - **RD2-L1/L2 (Low) — no test workspace-root signer/bundle reach the gate.** Python: **DONE** (`test_workspace_root_signer_and_bundle_resolve_from_root` — green). Rust: folded into the RD2-M1 agent.
- **RD2-M1 + RD2-L2 DONE (Rust, green):** `Workspace.index_trust_policy_explicit` added + parser sets it + `format_workspace_manifest` gates on it (parity with Python/Manifest); workspace-root signer/bundle-reach-gate test. 8 `Workspace{}` literal sites (7 already `..Default::default()`, 1 patched). RD2-L1 Python done.
- **REDESIGN COMPLETE. FINAL both-gates verification (my own runs): Python exit 0 · Rust exit 0 (re-run from repo root after a 127-cwd false-run I caught) · zero failure markers · no conflict markers · git identity corey@leavitt.dev, no local override.** **COMMITTED: 25bc246** `refactor(registry): root-scoped index-trust + close the workspace-discovery swallow cluster (#103 follow-on)` — 47 files, authored corey@leavitt.dev. (Combined onto 66f00ff review-fixes.) Working tree clean except the 3 intentional excludes.

**RD-M4 DONE (post-commit, uncommitted):** the "no manifest path to off" design question was already resolved BY the redesign (root can declare `index-trust "off"`; spec §3.4.7 line 618 says so) — stale note, no action. The remaining follow-up, RD-M4, is DONE: collapsed the ~12 `maybe_index` call-site glue repetitions in milpa-cli/src/main.rs into two wrappers `maybe_index_for_workspace(&ws)` / `maybe_index_for_manifest(&m)` (11 sites: 7 ws + 4 manifest; `cmd_verify` left inline — its extracted locals feed an offline/strict branch, not only maybe_index). Field extraction was already SSOT; this collapses the wiring so a future maybe_index signature change touches 2 helpers not 12. Behavior-preserving (existing suite green). Rust gate green, my own run from repo root (exit 0). Only main.rs changed. **Awaiting Corey commit go-ahead — recommend a separate `refactor(registry): collapse maybe_index trust-field glue (RD-M4)` commit** (25bc246 already clean; keep history honest).
All #103 index-trust follow-ons now resolved EXCEPT the genuinely-external: tianguis `index.kdl.bundle` delivery (cross-repo) · Rust real SigstoreVerifier (S4b / sigstore-rs#285) · Part-2 per-entry attribution (separate RFC).
- Pre-existing out-of-scope gap flagged by Phase 1: `format_manifest`/`format_workspace_manifest` don't round-trip index-trust fields (`add`/`remove` drop them) — predates this change; future serializer-completeness slice.
