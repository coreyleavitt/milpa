...
- **Stage:** 3 (slice grind) — **P1–P4 (main). S0 (main 94c524f). S1 (rust 6de9646). S2 harness+CI (rust a05697b). S3 manifest grammar (rust 2bf4fe5). S4 identity+CAS DONE+GREEN+COMMITTED (rust 09323f7). On `rust` branch. Next = S5a lockfile parse.**   •   **Round:** 2/2
...
- **S5a resume — lockfile PARSE** (RFC §6 S5a; depends on S4 for identity validation, NOT on S5b emit): in milpa-core `lockfile.rs`, implement the `LockfileParser` trait (`parse_lockfile(text) -> Lockfile`) reading `milpa.lock` KDL per `docs/spec/lockfile-schema.md`, mirroring `milpa/lockfile.py` parse path. Fan LOCK-* codes into CoreError (LOCK-KDL-SYNTAX already a placeholder; add the real granular slugs as raised) + grow all_codes(). Wire the `MilpaTarget::Cmd::ParseLockfile` arm in the conformance runner (currently unwired per S2) and REMOVE greened parse-lockfile fixture ids from known_failing.txt. **S5b (emit)** is a separate later slice. Spec refs: `docs/spec/lockfile-schema.md`, `docs/spec/errors.md` (LOCK-*), `milpa/lockfile.py`, RFC §6 S5a. Uses `kdl::parse_v1` (KDL 1.0, same as S3). TDD: port lockfile-parse unit tests + drive parse-lockfile error fixtures green.
