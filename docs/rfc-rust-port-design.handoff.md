...
- **Stage:** 3 (slice grind) — **P1–P4 (main). S0 (main 94c524f). S1 (rust 6de9646). S2 harness+CI (rust a05697b). S3 manifest grammar DONE+GREEN+COMMITTED (rust 2bf4fe5). On `rust` branch. Next = S4 identity+CAS.**   •   **Round:** 2/2
...
- **S4 resume — identity + CAS** (RFC §6 S4): in milpa-core `identity.rs` + `store.rs`, implement `compute_content_hash(path)` (sha256 of source tree per `docs/spec/identity.md` — strip-before-hash, sorted walk, exact byte algorithm) and the CaStore admit/link (copy-then-admit, NOT move; relative `_deps/<name>`→`.cas/sha256/<hex>` symlink; `cas_admissible` gate). Spec refs: `docs/spec/identity.md`, `milpa/identity.py` + `milpa/cas.py`, RFC §6 S4 + §9. S4 greens NO new conformance fixture on its own (first success fixture needs S4+S5b+S6+S7b+S7c+S9 per §9) — it unblocks them. Add CAS-* codes to CoreError::all_codes() as raised. TDD via ported identity.py unit tests (known content-hash vectors).
