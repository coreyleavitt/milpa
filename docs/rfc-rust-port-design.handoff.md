...
- **Stage:** 3 (slice grind) — **P1–P4 COMMITTED (main). S0 spikes DONE (decisions in RFC). S1 scaffold DONE+GREEN (uncommitted, Corey-gated). Next = commit S0→main + cut `rust` + S1→rust, then S2.**   •   **Round:** 2/2
- **Resume (stage 3):** `/loop implement the next unimplemented RFC slice with /tdd …` — next = **S2** (conformance harness RED backbone: `FixtureContext::load` + cmd dispatch + byte-diff/`expected/error` + urlkey + `<CAS_ROOT>` norm + error-parity `#[test]` vs errors.md + `known_failing.txt`; done = 2 synthetic fixtures green + minimal CI). **S1 onward lands on the `rust` branch, not `main`.**
