...
from harness.git_protocol_repo import _make_git_protocol_repo  # noqa: E402
...
# _make_git_protocol_repo moved to harness/git_protocol_repo.py (SSOT): the
# black-box differential runner (harness/runner.py) needs the SAME repo-
# generation recipe to drive the real CLI's ``fetch`` verb against locally
# generated repos, so the logic is factored into a module both import rather
# than maintained as two independent copies (CLAUDE.md audit-for-duplication
# discipline). Imported above alongside the other harness.* imports.
...
def _seed_cas(seed_root: Path, store: CAStore, scratch_root: Path) -> None:
    ...
    def _admit_tree(tree: Path, identity: str | None) -> None:
        """Stage ``tree`` and admit it. ``identity`` given → admit verbatim at
        that identity (identity-keyed layout, no re-hash); None → compute it."""
        staged = staging_root / tree.name
        if staged.exists():
            shutil.rmtree(staged)
        shutil.copytree(tree, staged)
        try:
            ident = identity if identity is not None else compute_content_hash(staged)
            if not store.contains(ident):
                store.admit(staged, ident)
        except Exception:
            pass
        finally:
            if staged.exists():
    ...
    for child in seed_root.iterdir():
        if not child.is_dir():
            continue
        # Identity-keyed layout: cas-seed/<algo>/<hex>/ mirrors the store and is
        # admitted verbatim at that identity (no re-hash). Required when dep-name
        # keying is ambiguous — two deps sharing one import name but differing in
        # identity (import-slot collision), or one tree backing several deduped
        # deps. See harness/runner.py _seed_cas_from_lock for the parallel path.
        if child.name in ("sha256", "dag-sha256"):
            for hex_dir in child.iterdir():
                if hex_dir.is_dir():
                    _admit_tree(hex_dir, f"{child.name}:{hex_dir.name}")
            continue
        # Dep-name-keyed layout: hash the tree and admit at the computed identity.
        _admit_tree(child, None)
