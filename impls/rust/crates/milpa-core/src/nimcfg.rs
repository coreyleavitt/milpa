...
pub fn format_nimcfg(
    ...
    for dep in &ordered {
        ...
        // --path: line here — its source directory is already emitted as
        // the self_src_dir line above (it IS the root); a --path: pointing
        // at "_deps/<name>" would reference a directory that is never
        // created (the root's tree is never staged into _deps/).
        if is_root_self_dep(dep) {
            continue;
        }
    ...
    if let Some(fd_map) = flag_defines {
        for dep in &ordered {
            if is_root_self_dep(dep) || dep.active_flags.is_empty() {
                continue;
            }
...
/// True when `dep` has a `ProvenanceRecord::Root` (§14 root-self entry).
/// Mirrors Python `_is_root_self_dep`.
fn is_root_self_dep(dep: &ResolvedDep) -> bool {
    dep.provenances.iter().any(|p| matches!(p, ProvenanceRecord::Root { .. }))
}
...
mod tests {
    ...
    /// §14 root-self entry: no identity, `Root`-kind provenance, no requires.
    fn root_self_dep(name: &str) -> ResolvedDep {
        ResolvedDep {
            ...
            src_dir: String::new(),
            requires: vec![],
            provenances: vec![ProvenanceRecord::Root {
                name: name.into(),
                origin: "observed".into(),
            }],
            dep_decl: None,
    ...
    /// §14.5: a `Root`-kind entry gets NO `--path:` line — its source is
    /// already the (always-first) self_src_dir line.
    #[test]
    fn root_self_dep_excluded_from_path_lines() {
        let graph = ResolvedGraph {
            deps: vec![root_self_dep("myapp"), dep("helper", "src")],
        };
        assert_eq!(
