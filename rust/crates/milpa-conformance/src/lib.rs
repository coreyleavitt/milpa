...
//! readers: this consumes the *same* `tests/conformance/spec-v<N>/` fixtures the
...
/// Resolved against `CARGO_MANIFEST_DIR` (= `rust/crates/milpa-conformance`) so
/// it is robust to CWD (RFC §4.6: anchor at the manifest dir, not a build
