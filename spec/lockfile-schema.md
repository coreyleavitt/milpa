...
13. Emit an OPTIONAL top-level `exclude_newer` node (see §2.2a) whenever the
    resolve ran under an effective `exclude-newer` time-bound; omit it
    entirely otherwise (never a sentinel timestamp). On `--frozen`/`frozen`,
    compare the lockfile's recorded `exclude_newer` against the manifest's
    EFFECTIVE `resolution { exclude-newer }` (default: unset) and raise
    `FROZEN-EXCLUDE-NEWER-MISMATCH` on any divergence. On `--locked`, treat a
    recorded `exclude_newer` that is absent (or different) from the new
    resolve's effective value as drift (`RES-LOCKED-DRIFT`) — dropping a
    previously-recorded bound relaxes semantics and MUST NOT happen silently.
...
(`exclude_newer` is OPTIONAL — see §2.2a; omitted entirely when no
time-bound was in effect.)
...
### 2.2a  `exclude_newer` node (additive, optional)

> NORMATIVE: A lockfile MAY contain at most one top-level `exclude_newer`
> node carrying exactly one string argument — an ISO 8601 timestamp — naming
> the EFFECTIVE `exclude-newer` time-bound (`resolver-semantics.md`
> Axis D) this lockfile was resolved under. Unlike `strategy` (required),
> this node is OMITTED ENTIRELY when no bound was in effect — never a
> sentinel or default timestamp. A conformant emitter writes this node
> exactly when the effective bound is non-`None`.

> NORMATIVE: A PRESENT-but-malformed `exclude_newer` node (wrong arity, a
> non-string argument, or an unparseable ISO 8601 value) MUST raise
> `LOCK-FIELD-ARITY` / `LOCK-FIELD-TYPE` as appropriate — the same
> arity/type-error convention `version` uses (§2.1), not a dedicated slug.
> Its ABSENCE is never an error (contrast with `strategy`'s
> `LOCK-STRATEGY-MISSING`) — an absent node means "no bound was recorded,"
> a legitimate and common lockfile state.

> NORMATIVE: `FROZEN-EXCLUDE-NEWER-MISMATCH` compares the lockfile's
> recorded `exclude_newer` against the manifest's EFFECTIVE `resolution {
> exclude-newer }` (built manifest-sourced from the start — the SAME shape
> `FROZEN-STRATEGY-MISMATCH`'s baseline uses, `manifest-grammar.md` §3.1).
> Any divergence (including "one side set, the other absent") is a
> mismatch.

> NORMATIVE (no-silent-drop, §6 D-D3 of `rfc-resolution-semantics.md`):
> dropping a previously-recorded `exclude_newer` RELAXES semantics (silently
> un-freezes the project) and MUST NOT happen silently.
> - `--locked` MUST treat "present in the committed lock, absent (or
>   different) in the new resolve's effective value" as drift
>   (`RES-LOCKED-DRIFT`), compared once for the whole lockfile (not per-dep).
> - `update`/`remove` (which have no CLI `--exclude-newer` surface) MUST
>   carry the prior lockfile's recorded `exclude_newer` forward into the
>   new resolve's effective value when the manifest does not declare its
>   own `resolution { exclude-newer }` — rather than silently reverting to
>   "no bound."
> - `fetch`/`lock` (which DO have a CLI `--exclude-newer` surface, `§2.11`
>   of `cli-contract.md`) are NOT required to carry forward — an absent CLI
>   flag and absent manifest declaration is a genuine "nothing declared
>   this run" result for these verbs; `--locked` is the guard that catches
>   an unintentional drop.
...
> NORMATIVE (resolution-semantics RFC §3 Axis A): for git/url/local/tarball/
> member dep kinds, `version` records the package's real **declared version**,
> derived by a fixed precedence over the fetched (or, for a workspace member,
> in-tree) package: (1) its own `milpa.kdl` `version` field; else (2) its
> `.nimble` `version`; else (3), git deps only, a version-shaped git ref tag
> (`v?X.Y.Z`); else (4) the dep declaration's `version=` annotation (or an
> `overrides { … version= }` rule's, for a redirected/purely-transitive dep).
> Named deps carry the actual resolved version string from the tianguis
> index directly (steps 1–4 do not apply — a named dep's version is never
> ambiguous). Version is a **label for constraint satisfaction only** — it is
> never an identity input; two dependency trees at the same declared version
> but different content remain distinct by `identity` (§3.1; see also
> `spec/identity.md §4.1a`).

> NORMATIVE: when none of steps 1–4 yields a version (**version-unknown**),
> a conformant emitter MUST write the absent-version literal `"0.0.0"` — and
> MUST omit the `declared_version_source` field (§3.2a) entirely for that
> dep. This exact pairing (`version "0.0.0"` + no `declared_version_source`)
> is reserved for the version-unknown case and MUST NOT be produced for a
> dep with a real declared version — see §3.2a for why this makes
> version-unknown unambiguous at the parse boundary. (The solver may use an
> internal decision token distinct from `"0.0.0"` while resolving a
> version-unknown dep; that token is discarded at the lockfile boundary in
> favor of this literal — an implementation detail, not part of the wire
> format.)

### 3.2a  `declared_version_source` field (additive, optional)

> NORMATIVE (resolution-semantics RFC §3 Axis A (b) / §5): `declared_version_source`
> is the sibling field to `version` — it records WHICH of the four precedence
> steps in §3.2 produced the declared version, as exactly one positional
> string argument: `"manifest"` (step 1 — the fetched package's own
> `milpa.kdl`), `"nimble"` (step 2), `"tag"` (step 3), or `"annotation"`
> (step 4). `"manifest"` names the *role* ("this package's own manifest"),
> not the file syntax of the day, so the value survives a future
> manifest-format evolution.

> NORMATIVE: `declared_version_source` MUST be emitted whenever `version`
> carries a real declared version (i.e., whenever the dep is not
> version-unknown per §3.2), and MUST be omitted when the dep is
> version-unknown. This is the one concrete flattening rule at this
> boundary: a version-unknown dep serializes to `version "0.0.0"` with no
> `declared_version_source` node, a combination no dep with a real declared
> version ever produces (every such dep names its source) — making
> version-unknown unambiguous to a parser with no other signal needed.
> `declared_version_source` never appears for a named/index-resolved dep
> (out of scope for the four-step precedence — its version comes straight
> from the index).

> NORMATIVE: an unrecognized value (e.g. from a future format revision) or
> malformed arity collapses to absent (`None`) at parse time rather than
> raising — the same conservative-collapse posture §3.9's `attestation
> { kind }` block takes. No dedicated error slug exists for this field; it
> is purely additive/diagnostic.

> NOTE: `declared_version_source` is never consulted by drift/`--locked`
> comparison, which keys on content hash + provenance (identity ⊥ version,
> per `spec/identity.md §4.1a`) — the one-time relabel of a previously
> sentinel-labeled git/url/local/tarball/member dep to its real declared
> version is therefore compatible, not drift.

> NOTE: additive field; no schema-epoch bump (unknown top-level and per-dep
> child nodes are already tolerated, §2 / §3).
...
| `FROZEN-EXCLUDE-NEWER-MISMATCH` | Manifest's effective `resolution { exclude-newer }` differs from lockfile's recorded `exclude_newer` (§2.2a) |
